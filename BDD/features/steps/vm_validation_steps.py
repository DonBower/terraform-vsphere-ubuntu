...
def step_connect_to_vsphere(context):
  context.si = SmartConnect(disableSslCertValidation=True, host=host, user=username, pwd=password)
...
def step_verify_template(context, template_name):
  ...
  assert context.vm.config.template == template_name, f"VM is not based on {template_name}, it's based on {context.vm.config.template}"

@then('the VM should have two disks')
def step_verify_disk_count(context):
  disk_count = sum(1 for device in context.vm.config.hardware.device if isinstance(device, vim.vm.device.VirtualDisk))
...
def step_verify_disk_size(context):
  for device in context.vm.config.hardware.device:
    if isinstance(device, vim.vm.device.VirtualDisk):
      assert device.capacityInKB <= 64 * 1024 * 1024, "First disk is larger than 64GB"
      break